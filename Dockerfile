...
ENV PATH=$APP_HOME/.local/bin:$PATH # Add local bin to PATH for Gunicorn if installed per user
...
COPY --from=builder /usr/local/bin/ /usr/local/bin/ # For global binaries like gunicorn, flask
